...
1. **Откройте панель объектов** — меню *Дополнительно — Объекты*. В панели отображается дерево объектов с текущими значениями. Двойной клик по объекту открывает его график.

2. **Просмотрите мнемосхемы** — меню *Схема*. Выберите доступную схему из списка.

3. **Проверьте состояние оборудования** — меню *Дополнительно — Оборудование*. Иконки слева от устройств показывают состояние связи.

4. **Создайте пользователей** — меню *Дополнительно — Пользователи*. Создайте учетные записи для операторов и задайте безопасный пароль администратору *root*. Смотрите [Конфигурирование пользователей](dev/users).
...
<dd>Нет связи с устройством или данные недостоверны. Проверьте состояние устройств в панели оборудования (<em>Дополнительно — Оборудование</em>).</dd>
