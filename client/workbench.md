...
- **Панель разделов** (слева) — две группы, разделённые линией.
  Верхняя группа переключает **режим боковой панели**: «Объекты» (дерево
  объектов и портфели), «Устройства» (дерево оборудования), «Файлы» (файлы
  и избранное) и «Узлы» (доступен только администратору). Режим меняет
  только состав боковой панели — он не открывает вкладок и не меняет
  страницу. Нижняя группа перечисляет **страницы** профиля пронумерованными
  кнопками (название страницы — во всплывающей подсказке): щелчок
  переключает страницу, кнопка «+» создаёт новую, перетаскивание меняет
  порядок, а правая кнопка мыши открывает меню «Переименовать / Удалить /
  Создать». Счётчик неквитированных событий выведен в строку контекста.
...
(*Настройки → Цветовая схема*). Значения и метки времени во всех таблицах отображаются
...
Экспериментальный интерфейс выбирается в меню *Настройки → Цветовая
схема* — рядом с пунктом *Стиль* и так же, как он: выбор применяется
сразу, без диалога и без перезапуска.
