...
Выделенная линия графика заполняет [инспектор](../workbench/#inspector-series):
под карточкой объекта появляется раздел «Серия» с палитрой цветов и текущими
режимами отображения линии — «Отдельная область», «Показывать точки»,
«Ступенчато». Цвет линии задаётся щелчком по образцу палитры; режимы
отображения в инспекторе только показываются, а переключаются командами
контекстного меню графика, описанными выше.
