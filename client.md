...
<dt>Страница</dt>
<dd>Команды для работы со страницами: создать, переименовать, дублировать или удалить страницу. Также содержит список всех страниц для переключения между ними. Подробнее — <a href="{{ '/client/workbench/#activity-bar' | relative_url }}">Панель разделов</a>.</dd>

<dt>Окно</dt>
<dd>Команды для работы с окнами текущей страницы. Позволяет переименовывать и закрывать открытые окна, а также помещать окна в избранное. Содержит список окон и позволяет переключаться между ними. Кроме того, содержит список последних закрытых окон, позволяя восстановить случайно закрытое окно.</dd>
...
<dd>Позволяет изменять внешний вид и поведение графического клиента.
  ...
  <dt>Показывать события при поступлении</dt>
  <dd>При поступлении нового неквитированного события автоматически открывает панель событий. Включено по умолчанию. Смотрите <a href="client/alarms">Оповещение о тревогах</a>.</dd>

  <dt>Скрывать события при квитировании</dt>
  <dd>Закрывает панель событий, когда квитировано последнее событие — как полученное от сервера, так и локальное сообщение самого Клиента. Включено по умолчанию.</dd>

  <dt>Мигание окна при событии</dt>
  <dd>Пока есть неквитированные события и окно Клиента неактивно, запрашивает внимание операционной системы: в Windows мигает кнопка на панели задач, в macOS подпрыгивает значок в Dock. Система снимает запрос, когда окно выводится на передний план. Выключено по умолчанию. Смотрите <a href="client/alarms">Оповещение о тревогах</a>.</dd>

  <dt>Звуковое оповещение при событии</dt>
  <dd>Пока есть хотя бы одно неквитированное событие, повторяет системный звуковой сигнал; сигнал прекращается после квитирования последнего события. Выключено по умолчанию. Работает только в Windows-сборке Клиента.</dd>
...
Центральная часть главного окна называется страницей. На странице могут быть открыты панели и рабочие окна. Допускается создание нескольких страниц с помощью команды меню `Страница - Новый`, при этом всегда отображается одна из них. Страницы можно переключать, выбирая соответствующее имя в меню `Страница`, или кнопками страниц на панели разделов. Имя страницы можно изменить командой `Страница - Переименование`, скопировать страницу вместе с её содержимым — командой `Страница - Дублировать`, удалить — командой `Страница - Удалить`.
...
Окна и панели могут располагаться на странице в произвольной разметке. Для перемещения окна по разметке следует перетащить его мышью за заголовок. При перетаскивании синим цветом будет показано новое расположение окна. Окна могут располагаться как рядом, с использованием горизонтального или вертикального разбиения, так и в «стопке» - в этом случае окна накладываются одно на другое, щелчок по заголовку позволяет переместить нужное окно на верх. При сохранении страницы разметка окон сохраняется, что позволяет переключать наборы отображаемых данных в зависимости от потребности.
...
- Опрос устройства (заставляет Сервер выполнить принудительный полный опрос устройства)

- Синхронизация часов (заставляет Сервер выполнить синхронизацию часов устройства с часами Сервера).</dd>
...
<dd>Открывает таблицу ретрансляции выделенного устройства-приёмника — список правил, по которым выбранные объекты передаются в другие системы верхнего уровня. Заполнение таблицы производится путем выбора объектов в окне дерева объектов; после этого необходимо задать адреса ретранслируемых объектов. Подробнее — <a href="{{ '/client/workbench/#transmission' | relative_url }}">Правила ретрансляции</a>:</dd>
...
Если для объекта включено [Двухэтапное Управление](dev/data-items), команда
передаётся на устройство в два этапа, в соответствии с МЭК-60870-5
(SELECT/EXECUTE):

1. **Выбор (SELECT).** Клиент посылает объекту команду выбора. Устройство
   резервирует выходную цепь под указанное значение, но ничего не переключает.
2. **Подтверждение оператором.** Запрос на подтверждение показывается только
   после успешного выбора, поэтому в нём сказано, что устройство уже готово
   исполнить команду. В запросе приводится текущее значение объекта, командное
   значение и предупреждение о необратимости действия:

   ![](img/control-select-confirm.png)

3. **Исполнение (EXECUTE).** По ответу «Да» посылается команда исполнения, и
   устройство переключает уже зарезервированную цепь.

По ответу «Нет» клиент посылает команду отмены, и выбор на устройстве
снимается сразу, а не по истечении собственного таймаута выбора устройства.
Драйверы, не поддерживающие отмену, отвечают на неё ошибкой — оператору эта
ошибка не показывается, так как выбор в этом случае всё равно снимется по
таймауту устройства.

При одноэтапном управлении команда выполняется одной операцией записи
значения, этапы выбора и исполнения не используются.
...
Панель объектов служит для быстрого выбора объектов, вызова различных окон и выполнения команд и операций с объектами. В панели объектов отображается структура объектов, разбитых по группам. Панель объектов можно отобразить командой меню *Дополнительно — Объекты*.

Дерево начинается сразу с объектов и групп верхнего уровня: отдельная корневая строка не выводится, поскольку заголовок панели уже называет её содержимое.
...
Для вызова панели Оборудование следует выбрать меню *Дополнительно — Оборудование*.

### Статусы устройств

В окне `Оборудование` отображается иерархия устройств, начинающаяся сразу с элементов верхнего уровня — отдельная корневая строка не выводится. Слева от устройств располагаются индикаторы текущего состояния канала связи с устройством (*канал связи отключен*, *канал связи включен* и *устройство не отвечает*):

![](img/hardware-tree.png)

Для вызова служебной информации о канале связи с устройством следует выбрать устройство и по клику ПКМ выбрать пункт меню *Метрики*:

![](img/device-metrics.png)
...
Смотрите [отображение модели МЭК-61850](protocols#iec-61850) и [проектирование](development) в Клиенте.

## [](#events-panel)Панель событий

Панель событий отображает список текущих не квитированных событий:

![](img/menu-events.png)

Для квитирования события нужно дважды кликнуть ЛКМ по строке события или выделить строку с событием и кликнуть по ней ПКМ, далее выбрать пункт всплывающего меню *Квитирование*. После квитирования, данное событие скрывается из панели событий. Также, удерживая клавишу *Shift*, можно выделить несколько событий и квитировать их одновременно. Для квитирования всех событий нажать кнопку *Квитировать все* на панели инструментов.

При включении опции *Показывать события при поступлении* в меню *Настройки* панель событий открывается сама при поступлении нового неквитированного события, а опция *Скрывать события при квитировании* закрывает её, когда квитировано последнее. Оба параметра, звуковое оповещение и порог важности описаны на странице [Оповещение о тревогах](client/alarms).
...
Для переименования файла или папки необходимо использовать команду `Свойства` и отредактировать имя объекта (имя файла).


## Сводка

Смотрите [Сводка](client/summary).

## Данные

Смотрите [Данные](client/data).

## Метрики устройств

Смотрите [Метрики устройств](client/device-metrics).

## Журнал событий

Смотрите [Журнал событий](client/events).
...
Смотрите [Наблюдение](client/device-watch).

## Отладчик

Смотрите [Отладчик](client/debugger).
