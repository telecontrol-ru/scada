...
<dt>Период опроса</dt>
<dd>Группа определяет периоды отправки команды полного опроса (General Interrogation) и период опроса каждой группы МЭК отдельно. (Описание групп смотрите в стандарте МЭК для команды полного опроса.) Если значение поля равно 0, то опрос производится не будет.</dd>
